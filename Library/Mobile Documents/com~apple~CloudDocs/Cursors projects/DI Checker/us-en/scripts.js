...
function addDrugToCheck(drugName) {
    ...
    if (!drug1.value) {
        ...
        // Move focus to second drug input field after adding first drug
        setTimeout(() => {
            drug2.focus();
        }, 500);
        ...
        utils.showAlert(`${drugName} has been added as the second drug. You can now check the interaction.`, 'success');
        
        // Automatically suggest interaction check after adding second drug
        setTimeout(() => {
            const shouldProceed = confirm(`Would you like to check the interaction between ${drug1.value} and ${drugName} now?`);
            if (shouldProceed) {
                checkInteraction();
            }
        }, 1000);
        ...
        // When two drugs are already selected - provide option to replace one
        const options = [
            `Replace first drug (${drug1.value}) with ${drugName}`,
            `Replace second drug (${drug2.value}) with ${drugName}`,
            'Cancel'
        ];
        
        const choice = prompt(`Two drugs are already selected.\n\n1: ${options[0]}\n2: ${options[1]}\n3: ${options[2]}\n\nEnter a number (1, 2, 3):`);
        
        if (choice === '1') {
            drug1.value = drugName;
            utils.showAlert(`First drug has been replaced with ${drugName}.`, 'success');
        } else if (choice === '2') {
            drug2.value = drugName;
            utils.showAlert(`Second drug has been replaced with ${drugName}.`, 'success');
        } else {
            utils.showAlert('Drug addition cancelled.', 'info');
            return;
        }
    }
...
function selectDrug(inputId, drugName) {
    try {
        ...
        if (!inputElement) {
            ...
            utils.showAlert('System error: Input field not found.', 'warning');
            return;
        }
        
        if (!drugName || typeof drugName !== 'string') {
            console.error('❌ Invalid drug name:', drugName);
            utils.showAlert('Invalid drug name.', 'warning');
            return;
        }
        
        const sanitizedDrugName = SecurityUtils.sanitizeInput(drugName.trim());
        if (!sanitizedDrugName) {
            utils.showAlert('Please enter a valid drug name.', 'warning');
            return;
        }
        
        // Set drug name in input field
        ...
        const fieldType = inputId === 'drug1' ? 'first' : 'second';
        utils.showAlert(`${sanitizedDrugName} has been selected as the ${fieldType} drug.`, 'success');
        
        // Auto focus to second drug input after selecting first drug
        if (inputId === 'drug1') {
        ...
        // Suggest interaction check when second drug is selected
        if (inputId === 'drug2') {
            const drug1Element = document.getElementById('drug1');
            if (drug1Element && drug1Element.value) {
                setTimeout(() => {
                    const shouldProceed = confirm(`Would you like to check the interaction between ${drug1Element.value} and ${sanitizedDrugName} now?`);
                    if (shouldProceed) {
                        checkInteraction();
                    }
                }, 800);
        ...
        console.error('❌ selectDrug error:', error);
        utils.showAlert('An error occurred during drug selection.', 'warning');
    }
}
...
async function checkInteraction() {
    ...
    if (!drug1Element || !drug2Element) {
        ...
        utils.showAlert('System error: Drug input fields not found.', 'warning');
        return;
    }
    ...
    console.log('🔍 Drug information entered:', {
        drug1: drug1,
        drug2: drug2,
    ...
    if (!drug1 && !drug2) {
        utils.showAlert('Please enter both drugs.\n\n💡 Search and select drugs or enter them directly.', 'warning');
    ...
    if (!drug1) {
        utils.showAlert('Please enter the first drug.', 'warning');
        drug1Element.focus();
        return;
    }
    
    if (!drug2) {
        utils.showAlert('Please enter the second drug.', 'warning');
        drug2Element.focus();
        return;
    }

    if (drug1 === drug2) {
        utils.showAlert('Please select different drugs.\n\nThe same drug is currently selected.', 'warning');
        drug2Element.focus();
        drug2Element.select();
        return;
    }
