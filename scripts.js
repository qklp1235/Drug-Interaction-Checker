...
// API Server URL (change according to development/production environment)
...
async function generateOneTimeApiKey() {
    try {
        ...
        utils.showLoading('Generating one-time API key...');
        ...
        if (!response.ok) {
            throw new Error(`API key generation failed: ${response.status}`);
        }
        
        const data = await response.json();
        
        if (data.success) {
            // Auto-fill generated API key to Perplexity field
            ...
            document.getElementById('remainingUses').textContent = `${data.dailyLimit} times`;
            ...
            utils.hideLoading();
            utils.showAlert('🎉 One-time API key generated successfully!', 'success');
            
            // Auto-change AI provider
        ...
        utils.hideLoading();
        utils.showAlert(`❌ API key generation failed: ${error.message}`, 'error');
        
        // Restore button
        const generateBtn = document.getElementById('generateKeyBtn');
        generateBtn.disabled = false;
        generateBtn.textContent = '🎫 Generate One-time API Key';
    }
}

// Check one-time API key status
async function checkOneTimeApiStatus() {
    try {
        const apiKey = SecurityUtils.secureLocalStorage.getItem('one_time_api_key');
        if (!apiKey) {
            utils.showAlert('❌ No saved one-time API key found.', 'error');
            return;
        }
        
        utils.showLoading('Checking usage...');
        ...
        if (!response.ok) {
            if (response.status === 401) {
                ...
                utils.hideLoading();
                utils.showAlert('❌ API key has expired. Please generate a new key.', 'error');
        ...
        // Update usage
        document.getElementById('remainingUses').textContent = `${data.remaining} times`;
        SecurityUtils.secureLocalStorage.setItem('one_time_api_usage', data.usage.toString());
        
        utils.hideLoading();
        utils.showAlert(`📊 Usage check complete: ${data.usage} times used, ${data.remaining} times remaining`, 'info');
        
    } catch (error) {
        console.error('API key status check error:', error);
        utils.hideLoading();
        utils.showAlert(`❌ Status check failed: ${error.message}`, 'error');
    }
}
...
function copyApiKey() {
    try {
        ...
        if (!apiKey) {
            utils.showAlert('❌ No API key to copy.', 'error');
            return;
        }
        
        // Copy to clipboard
        navigator.clipboard.writeText(apiKey).then(() => {
            utils.showAlert('📋 API key copied to clipboard!', 'success');
        ...
        utils.showAlert('❌ Error occurred while copying API key.', 'error');
    }
}

// Reset one-time API UI
...
// Check one-time API key status and update UI
function checkOneTimeApiKeyStatus() {
    ...
    if (generateBtn && checkStatusBtn && resultDiv) {
        ...
        if (expiresAtSpan) {
            expiresAtSpan.textContent = expiryDate.toLocaleString('en-US');
        }
        ...
        if (remainingUsesSpan) {
            remainingUsesSpan.textContent = `${Math.max(0, 5 - parseInt(usage))} times`;
        }
    }
}

// Override existing AI call function to integrate with one-time API key
const originalCallPerplexity = utils.callPerplexity;
utils.callPerplexity = async function(messages, options = {}) {
    try {
        // Check one-time API key
        ...
        if (oneTimeApiKey && expiresAt) {
            ...
            if (now <= expiryDate) {
                // Call through server using one-time API key
                ...
                if (!response.ok) {
                    if (response.status === 429) {
                        throw new Error('Daily usage limit exceeded. (5 times per day)');
...
// Check one-time API key status on page load
document.addEventListener('DOMContentLoaded', function() {
